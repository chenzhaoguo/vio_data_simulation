...
MotionData IMU::MotionModel(double t) {
  ...
  Eigen::Vector3d eulerAnglesRates(-k_roll * sin(t), k_pitch * cos(t), K);  // eulerAngles的导数

  // Eigen::Vector3d eulerAngles(0.0, 0.0, K*t);  // roll ~ 0, pitch ~ 0, yaw ~ [0,2pi]
  // Eigen::Vector3d eulerAnglesRates(0.0, 0.0, K);  // eulerAngles的导数
...
void IMU::testImu(std::string src, std::string dest) {
  ...
  for (int i = 1; i < imu_data.size(); ++i) {
    MotionData imu_pose = imu_data[i];

    // delta_q = [1, 1/2 * theta_x, 1/2 * theta_y, 1/2 * theta_z]
    Eigen::Quaterniond dq;
    Eigen::Vector3d dtheta_half = imu_pose.imu_gyro * dt / 2.0;
    dq.w() = 1;
    dq.x() = dtheta_half.x();
    dq.y() = dtheta_half.y();
    dq.z() = dtheta_half.z();
    ...
    // imu 动力学模型 欧拉积分
    Eigen::Vector3d acc_w = Qwb * (imu_pose.imu_acc) + gw;  // aw = Rwb * (acc_body - acc_bias) + gw
    Qwb = Qwb * dq;
    Vw = Vw + acc_w * dt;
    ...
    save_points << imu_pose.timestamp << " "
                << Qwb.w() << " "
                << Qwb.x() << " "
                << Qwb.y() << " "
                << Qwb.z() << " "
